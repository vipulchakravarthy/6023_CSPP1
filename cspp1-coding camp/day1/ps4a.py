...
def displayHand(hand):
    ...
    for letter in hand.keys():
        for j in range(hand[letter]):
            letters += " " + letter       # print all on the same line
...
def isValidWord(word, hand, wordList):
    ...
    counter = 0
    for letter in word:
        if letter in temp and temp[letter] >= 1:
            temp[letter] -= 1
            counter+= 1
    if  counter == len(word) :
        if word in wordList:
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        if word_input == '.':
            ...
            if isValidWord(word_input , hand, wordList):
                score_word = getWordScore(word_input, n)
                print(word_input, " earned ", score_word, "points")
